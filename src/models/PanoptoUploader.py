...
class PanoptoUploader:
    ...
    def __check_retry_needed(self, response):
        ...
        True indicates retry needed, False indicates success. Otherwise an exception is thrown.
        """
        LOGGER.debug("")
        ...
        if response.status_code == requests.codes.forbidden:
            ...
            self.__setup_or_refresh_access_token()  # NEW JBRUYLANT 280621
            return True
    ...
    def __create_manifest_for_video(self, file_path, date_created, manifest_file_name):
        ...
        content = (
            ...
            .replace(
                "{Description}",
                f"Created: {date_created[0]} => Transferred to Panopto:  {dt.datetime.today().strftime('%Y-%m-%d')}",
            )
            .replace("{Filename}", file_name)
            .replace("{Date}", dt.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f-00:00"))
        )
    ...
    def __monitor_progress(self, upload_id):
        ...
        previous_state = None
        endTime = dt.datetime.now() + dt.timedelta(hours=globalConfig.PanoptoCountdown)
        while True:
            time.sleep(20)
            url = f"https://{self.server}/Panopto/PublicAPI/REST/sessionUpload/{upload_id}"
            ...
            if state != previous_state:
                ...
                LOGGER.debug(" State: {0}".format(UPLOAD_STATUS.get(state, "Unknown State")))
        ...
        upload_status = {
            "Status_Value": state,
            "Status_Name": UPLOAD_STATUS.get(state, "Unknown State"),
        }
        return upload_status
